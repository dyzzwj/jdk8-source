...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     */
    /**
     * ctl是对线程池的运行状态(runState)和线程池中有效线程的数量（workCount）进行控制的一个字段，
     ...
     * ctl一个32位 低29位记录workerCount 高3位记录runState
     */
     ...
     * 在线程池处于 RUNNING 状态时，调用 shutdown()方法会使线程池进入到该状态。（finalize() 方法在执行过程中也会调用shutdown()方法进入该状态）
     */
     ...
     * expire.
     *
     *  任务队列 当任务提交时，如果线程池中的线程数量大于等于corePoolSize的时候，把该任务封装成一个Worker对象放入等待队列；
     */
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            /**
             * 如果runState是非正常运行状态 并且 （runState不是SHUTDOWN 或 firstTask不为空 或  任务队列为空）
             */
            ...
            if (t != null) {
                ...
                try {
                    ...
                    if (rs < SHUTDOWN ||
                        ...
                        if (t.isAlive()) //预检查线程还未启动  precheck that t is startable
    ...
    public void execute(Runnable command) {
        ...
        if (isRunning(c) && workQueue.offer(command)) {
            ...
            if (!isRunning(recheck) && remove(command)) {
                // 任务添加到阻塞队列失败，执行拒绝策略
                reject(command);
            } else if (workerCountOf(recheck) == 0) {  // 如果线程池还是 RUNNING 的，并且线程数为 0，那么开启新的线程
                addWorker(null, false);
            }
